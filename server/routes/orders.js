...
// GET all main orders with sub-orders
...
// POST create order — with validations
router.post('/', async (req, res) => {
  const { vehicle_id, customer_id, mechanic_id, description, parent_id } = req.body
  if (!vehicle_id || !customer_id || !description)
    return res.status(400).json({ error: 'vehicle_id, customer_id and description are required' })

  const client = await pool.connect()
  try {
    await client.query('BEGIN')

    // Validation 1 — check car is not inactive
    const { rows: vehRows } = await client.query(
      `SELECT car_status FROM vehicles WHERE id=$1`, [vehicle_id]
    )
    ...
    // Validation 2 — no duplicate active order for same vehicle (main orders only)
    if (!parent_id) {
    ...
    // Validation 3 — no duplicate sub-order description under same parent
    if (parent_id) {
      const { rows: parentRows } = await client.query(
        `SELECT status FROM orders WHERE id=$1`, [parent_id]
      )
    ...
    // Auto-set vehicle to in-repair when a main order is created
    if (!parent_id) {
      await client.query(
        `UPDATE vehicles SET car_status='in-repair' WHERE id=$1`, [vehicle_id]
      )
    }
...
// PATCH update order status — with validations
router.patch('/:id/status', async (req, res) => {
  const { status } = req.body
  const valid = ['pending', 'progress', 'done']
  ...
  try {
    ...
    // Validation — cannot mark as done if sub-orders are still pending/in-progress
    ...
    // Auto-update car status when main order is done
    if (status === 'done' && !order.parent_id) {
      await client.query(
        `UPDATE vehicles SET car_status='active' WHERE id=$1`, [order.vehicle_id]
      )
    }

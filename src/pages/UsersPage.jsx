...
export default function UsersPage() {
  ...
  const openEdit = (u) => { setForm({ name: u.name, email: u.email, password: u.password, role: u.role }); setEditing(u); setFormError(''); setShowForm(true) }
  ...
  const handleSave = () => {
    ...
    const emailExists = localUsers.some(u => u.email === form.email && u.id !== editing?.id)
    if (emailExists) { setFormError('Email already in use.'); return }

    if (editing) {
      setLocalUsers(prev => prev.map(u => u.id === editing.id ? { ...u, ...form } : u))
    } else {
      setLocalUsers(prev => [...prev, { ...form, id: Date.now() }])
    }
    setShowForm(false); setEditing(null)
  }

  const handleDelete = () => {
    setLocalUsers(prev => prev.filter(u => u.id !== confirmDel.id))
    setConfirmDel(null)
  }
    ...
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#6b6b68', marginTop: 2 }}>{localUsers.length} users</p>
        </div>
        ...
        {localUsers.map(u => (
          <div key={u.id} style={{
            ...
            <span style={{
              ...
              fontWeight: 500, display: 'inline-block', textTransform: 'capitalize',
              ...ROLE_STYLE[u.role],
            }}>{u.role}</span>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: '1.5rem', maxWidth: 400, width: '100%', border: '0.5px solid rgba(0,0,0,0.1)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button onClick={handleSave} style={{ fontSize: 13, padding: '7px 14px', borderRadius: 8, border: 'none', background: '#185FA5', color: '#fff', cursor: 'pointer', fontFamily: 'inherit' }}>Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {confirmDel && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: '1.5rem', maxWidth: 360, width: '100%', border: '0.5px solid rgba(0,0,0,0.1)' }}>
            ...
            <p style={{ fontSize: 13, color: '#6b6b68', marginBottom: '1.25rem' }}>This will remove <strong>{confirmDel.name}</strong> permanently.</p>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
